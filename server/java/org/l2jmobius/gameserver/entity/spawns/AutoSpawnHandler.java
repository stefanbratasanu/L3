...
public class AutoSpawnHandler
{
	...
	{
		final List<AutoSpawnInstance> result = new ArrayList<>();
		for (AutoSpawnInstance spawnInst : _registeredSpawns.values())
		{
		...
		public void run()
		{
			try
			{
				...
				if (npcInst != null)
				{
					if (spawnInst.isRandomSpawn())
					{
						while ((World.getFirstVisibleObjectInRange(npcInst, Npc.class, 5) != null))
						{
					...
					{
						World.broadcastToAllOnlinePlayers("The " + npcInst.getName() + " has spawned near " + MapRegionData.getInstance().getClosestTownName(npcInst) + "!");
					}
				}
